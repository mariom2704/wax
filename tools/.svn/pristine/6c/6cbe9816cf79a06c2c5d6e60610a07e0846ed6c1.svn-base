...
/**
Liste um Objekte vom Typ MxBase zu speichern. 
...
public:
	...
	void append(MxBase *newElement);				///< neues Element an die Liste anhängen

	void appendListe(MxListe& liste);				///< Liste anhängen

	void insertAt(int pos,MxBase *newElement);		///< neues Element an der Stelle pos in die Liste einfügen
	...
	MxBase* getAt(const MxIterator &iterator)const;	///< Zeiger auf Element  

	MxBase* getFirst()const;						///< Zeiger des 1. Elementes in der Liste zurückgeben

	MxBase* getLast()const;							///< Zeiger des letztes Elementes in der Liste zurückgeben

	MxBase* getAtIndex(int index);					///< Zeiger auf Element in Lista an Stelle index


	bool setAt(const MxIterator &iterator,MxBase *newElement);	///< Element an der Position pos durch das Element newElement ersetzen.
...
*/
class  MxIterator :	public MxBase
{
public:
	MxIterator();
...
/*
Beispile für die Benutzung der Klasse
