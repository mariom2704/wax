...
{ 
	...
	std::vector< std::pair<std::string,bool> > searchstr;

	// Hier die Suchreihenfolge festlegen
	if (office2010_mode)
	{
		searchstr.push_back( std::make_pair<std::string, bool>("*.xlsx", true) );
		searchstr.push_back( std::make_pair<std::string, bool>("(*.xls)", false) );
	}
	...
	for (std::vector< std::pair<std::string, bool> >::iterator it = searchstr.begin(); it != searchstr.end(); it++)
	{
		do {
			if( strstr( pszBuf, it->first.c_str() ) != 0 ) 
			{// Found !
				TRACE("ExcelTreiber: %s\n", pszBuf);
				office2010_mode = it->second;
