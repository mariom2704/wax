...
#define USERVAR         "NWUSERNAME" /* environment variable for userid */
...
if (getenv(USERVAR)!=NULL) 
{
	strncpy(userid, getenv(USERVAR), 512);
}
else if ( GetUserName(uname, &ulen) )
{
	strncpy(userid, (LPCSTR)uname, ulen);
}
...
/* hIcon = CreateIcon(hInst, 32, 32, 1, 1, ANDmaskIcon, XORmaskIcon);*/
 hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_IDENTD3));
...
/* ShowWindow(hwnd, SW_SHOW); */
 ...
 sprintf(nid.szTip, "Sie sind als '%s' angemeldet (%i Verbindungen)", userid, connect_count);
 ...
 while ( !TimeToQuit ) {
  ...
  for (user=head; user!=NULL; user=(CONNECTION*)user->next)
  ...
  if ( FD_ISSET(listen_sock, &readmask) ) {
   ...
   if ((user=(CONNECTION*)create_user())==NULL) {
   ...
   user->sock=(int)accept_sock;
   user->last_input=(int)time((time_t *)0);
  }
  ...
  while( user != NULL ) {
   ...
   for (user=head; user!=NULL; user= (CONNECTION*)user->next) {
    ...
    user->last_input=(int)time((time_t *)0);  /* ie now */
...
CONNECTION *i, *j;
 ...
 if (user==head) {
  ...
  head=(CONNECTION*)head->next;
  free(user);
  return 1;
 }
 for (i=head; (CONNECTION*)i->next!=user && i->next!=NULL; i=(CONNECTION*)i->next);
 j=(CONNECTION*)i->next;
 if (j!=user) {
...
CONNECTION *new_user;
 ...
 if (head==NULL) {
  head=(CONNECTION*)malloc(connect_str_size);
 ...
 new_user=(CONNECTION*)malloc(connect_str_size);
...
void write_connection(CONNECTION *user, char *str) {
 if (user==NULL) return;
 if (user->sock>-1) send(user->sock, str, (int)strlen(str), 0);
}
