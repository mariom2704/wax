...
CString Rohrleitung::copy(const Rohrleitung& other)
{
...
void Rohrleitung::serialize(BinaryArchive& ar)
{
	...
	if (ar.is_storing())
	{
		ar << dnvl << laenge << nRohre << rohrQuali << rohrVerb;
...
CString Rohrleitung::getKappenfarbe(int farbe)
{
	...
	switch (farbe)
	{
		...
		case GOLD:		ids = "IDS_FARBE_GOLD";		break;
		case WEISSALU:	ids = "IDS_FARBE_WEISSALU";	break;
		case PINK:		ids = "IDS_FARBE_PINK";		break;
		case BRAUN:		ids = "IDS_FARBE_BRAUN";	break;
		case ORANGE:	ids = "IDS_FARBE_ORANGE";	break;
		case LILA:		ids = "IDS_FARBE_LILA";		break;
...
bool Rohrleitung::makeRohre(EnCObListModell& elementList)
{
	...
	{ 
		...
		if (nRohre > 1)
		{
			...
			int next = (i==1)?i+1:i;
			if (rohrLaengen[next]-rohrEndeLaenge > 0 || nRohre == 2)
			{
				// zu "(rohrLaengen[i]==0 || rohrLaengen[0]==0 && i==2)?LEER:SCHWEISS":
				// Bogen in der Mitte		- nach Bogen kommt SCHWEISS
				// Bogen am Anfang			- nach Bogen kommt LEER
...
void Rohrleitung::check_rohrQuali(int dnvl) const
{
	...
	switch (rohrQuali)
	{
		...
		case RQ2458GSCH_DICK:
		if(dnvl != 65)
		{
				str = getStammdaten().getText("IDS_ERROR_190").c_str();  // und DN 32/40 und Länge > 9000 ungültig!
...
// Relativen Abstand prüfen
		...
		if (ele->get_abstRel() == 0 && !ele->isEleRed())
		{
				      ...
				      || ele->isEleBogen())
			{
				...
				{
					if(prev->get_id() != BOHR50 &&  prev->get_id() != ROHRNIPPEL && ele->get_id() != SPRINKLERSCHELLE_VIROTEC && prev->get_id() != SPRINKLERSCHELLE_VIROTEC)
					{
						err = getStammdaten().getText("IDS_FEHLER_GLEICHER_WINKEL_SELBE_POS"); //Relativer Abstand 0 nicht zulässig!
						throw (CString)(strx + err).c_str();
					}
...
// Absoluten Abstand vom Anfang und Ende überprüfen
		...
		if (ele->get_id() == FFL16 && ele->get_dn() < 50) 
		{
			throw CString( getStammdaten().getText("IDS_ERROR_345").c_str());
		}
...
void Rohrleitung::check_biegung()const
{
	...
	for (int i=0; i < elementArray.GetSize(); i++)
	{
		...
		{
			err.Format(getStammdaten().getText("err_biegung9").c_str(), minAbst);
			throw err; //Der Mindestabstand zwischen 2 Biegunggen
		}
