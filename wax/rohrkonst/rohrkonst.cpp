...
static AFX_EXTENSION_MODULE RohrkonstDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ROHRKONST.DLL Initializing!");
		TRACE(" hInstance = 0X%x\n", hInstance);
		
		// One-Time-Initialisierung der Erweiterungs-DLL
		if (!AfxInitExtensionModule(RohrkonstDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RohrkonstDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ROHRKONST.DLL Terminating!\n");
		// Bibliothek vor dem Aufruf der Destruktoren schließen
		AfxTermExtensionModule(RohrkonstDLL);
	}
	return 1;   // OK
}
