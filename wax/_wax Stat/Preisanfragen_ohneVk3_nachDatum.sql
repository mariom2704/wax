...
where (select max(s.freigabedatum) from waxserver.status s where waid =  (select max(waid) from waxserver.werkauftrag w where p.nr = w.preisanfrageNr)) >= *WaxDateVon*
