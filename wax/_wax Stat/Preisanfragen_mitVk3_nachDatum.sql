...
where (select max(s.status) from waxserver.status s where waid = (select max(waid) from waxserver.werkauftrag w where p.nr = w.preisanfrageNr)) >= 3
...
and (select max(s.freigabedatum) from waxserver.status s where waid =  (select max(waid) from waxserver.werkauftrag w where p.nr = w.preisanfrageNr)) >= *WaxDateVon*
and (select max(s.freigabedatum) from waxserver.status s where waid =  (select max(waid) from waxserver.werkauftrag w where p.nr = w.preisanfrageNr)) <= *WaxDateBis*
