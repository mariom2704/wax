...
void CInPlaceEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if ((nChar == VK_RIGHT || nChar == VK_LEFT))
    {
		...
		WORD hh = HIWORD(dwSel);
		WORD lo = LOWORD(dwSel);
		if(hh == 0 && lo == 0 && nChar == VK_LEFT)
		{
			m_nLastChar = nChar;
			GetParent()->SetFocus();
			return;
		...
		CString strMyText; 
		GetWindowText(strMyText);
		int iLength = strMyText.GetLength();
		if( hh == iLength && lo == iLength && nChar == VK_RIGHT )
		{
			m_nLastChar = nChar;
			GetParent()->SetFocus();
			return;
