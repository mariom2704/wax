...
static AFX_EXTENSION_MODULE WaxuiDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("WAXUI.DLL Initializing!\n");
		
		// One-Time-Initialisierung der Erweiterungs-DLL
		if (!AfxInitExtensionModule(WaxuiDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(WaxuiDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("WAXUI.DLL Terminating!\n");
		// Bibliothek vor dem Aufruf der Destruktoren schließen
		AfxTermExtensionModule(WaxuiDLL);
	}
	return 1;   // OK
}
