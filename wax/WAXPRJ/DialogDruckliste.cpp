...
DialogDruckliste::DialogDruckliste(DialogHtmlDruck& file, WaxPart* pt) : waxpart(pt), html(file)
{
...
DialogDruckliste::DialogDruckliste(DialogHtmlDruck& file, CObList* ptWerkaufList)	: html(file)
{
	dn2zoll = (WaxSys::para_int(GF_DN2ZOLL) == 0) ? false : true;
...
DialogDruckliste::~DialogDruckliste()
{
}

void DialogDruckliste::init_auftragsliste()
{
	auftraege.clear();
	if (!waxpart)
		return;
...
int DialogDruckliste::scan_maxlaenge() const
{
	int maxlen = 0;
...
void DialogDruckliste::sort_liste(CObList &liste, bool aufsteigend)
{
...
*/
void DialogDruckliste::insert_rohr_quali(const Rohrleitung* rohr, int stutzenAnlage2440)
{
	if (!rohr)
		return;
...
bool DialogDruckliste::loadResource(CString logo, HRSRC rclog, HMODULE module)
{
