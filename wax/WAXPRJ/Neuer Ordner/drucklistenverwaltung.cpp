...
const CString DrucklistenVerwaltung::makeAusgabe(int id)
{
	...
	// Wenn Auftrag über Verteileranlage, dann in Saegelisten vermerken (Leitkarte - Ziel)
	...
	else
	{
		...
		SammelWerkauftrag* sammel =  dynamic_cast<SammelWerkauftrag*>(cur_werkauftrag());

		Werkauftrag* werk = NULL;
		if(sammel != NULL)
		{
			for (POSITION pos = walist.GetHeadPosition(); pos != NULL; )
			{
				SammelWerkauftrag* sammel = dynamic_cast<SammelWerkauftrag*>(walist.GetNext(pos));
				if(sammel != NULL)
				{
					werk = dynamic_cast<Werkauftrag*>(walist.GetNext(pos));
					break;
				}
			}
		}
		...
		{
			werk = cur_werkauftrag();
		}
	
		html.insertOberflaeche(werk, cur_werkauftrag()->get_beschichtung(), cur_werkauftrag()->get_zeugnis(), 
							   cur_werkauftrag()->get_sichtbereich(), cur_werkauftrag()->get_KollilisteExport());
	}
...
void DrucklistenVerwaltung::create_drucklist(WaxPart* curwaxpart)
{
	switch (currentAusgabe)
	{
		...
		default: 
			// @todo: Alle Drucklisten in eigene Klassen auslagern bis diese beiden Methoden überflüssig sind 
			initListen();
			writeList();
			break;
	}
...
bool DrucklistenVerwaltung::insertGrafic()
{
	...
	*/
	...
	if (WaxSys::isAV() && !cur_werkauftrag()->get_posNr().IsEmpty())
	{
		...
		if(m_bWidthJScript)
		{
			JScriptVisualisationAndPrinting.createBarcode(100, 20, cur_werkauftrag()->get_posNr().GetString(), "LEFT");
		}
		else
		{
