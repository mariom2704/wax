...
namespace WaxDruck
{
	...
	*/
	class WAXPRJDLL MehrfachDialogDrucklistenVerwaltung
	{
	/// public
	public:
		...
		MehrfachDialogDrucklistenVerwaltung(MxCObList* pt, bool bWidthJScript= false);

		/// Destruktor
		virtual ~MehrfachDialogDrucklistenVerwaltung();

		/// Erzeugt die gewünschte Ausgabedatei
		const CString makeAusgabe(int& iHowManyToPrint, CStringArray* aPath, int id = 0);
	...
	private:
		...
		void write_header(CString writeHeader, int i);
