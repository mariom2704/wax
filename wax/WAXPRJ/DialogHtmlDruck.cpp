...
DialogHtmlDruck::DialogHtmlDruck()
{}

DialogHtmlDruck::DialogHtmlDruck(int i):file(i)
{
}


DialogHtmlDruck::DialogHtmlDruck(CString fn)
:file(&fn, "")
{}
...
void DialogHtmlDruck::insertZeiten(double schweisserei, double pulveranlage)
{
    CString txt;
...
*/
void DialogHtmlDruck::insertZeile2Spalten(const CString &spalte1,int breiteSpalte1,bool center1, const CString &spalte2,int breiteSpalte2,bool center2)
{
	CString align;
...
*/
void DialogHtmlDruck::insertZeile4Spalten(const CString &spalte1,int breiteSpalte1,bool center1, 
...
void DialogHtmlDruck::insertOberflaeche(Werkauftrag* werkauft, const Beschichtung &beschichtung, BOOL zeugnis, BOOL sichtbereich, int iKolliliste)
{
    /*
	...
	*/
	...
	if(werkauft->get_GUID().IsEmpty() == FALSE)
	{
		oberfl = "";
	}
...
void DialogHtmlDruck::insertKopf(const CString &ueberschrift1, const CString &ueberschrift2,
							...
							bool umbruch,const CString &nichtGesetzt)
{
	CString stWert,
		    out,
			head,
...
void DialogHtmlDruck::WriteCuttingHeader(CString Name)
{
	...
	"\n<center>\n<table border=\"1\" cellspacing=\"0\" cellpadding=\"3\" width=\"100%\">\n"
		...
		"<tr>"
			// 1. spalte
			"<td width=\"30%\" valign=\"middle\" colspan=\"1\">\n"
			"<p align=\"center\"><img src=\"minimax.jpg\" width=\"155\" height=\"23\"></img></p></td>\n"
			...
			// 3. spalte
			"<td width=\"20%\" valign=\"middle\" colspan=\"1\">\n<font face=\"arial\" size=\"4\"><p align=\"center\"><div id=\"ele" 
			+  CString("")
			+"\" align=\"center\">" 
				+ CString("")							// z.b. versand
				+ "</div></p></font></td></tr>\n";
...
//Fuegt die Leitkarte in eine Druckliste ein
void DialogHtmlDruck::insertLeitkarte( Leitkarte& leitkarte, Status& status, const CString& name, const CString & ziel, int i, bool umbruch)
{
		/*
	...
	*/
	...
	CString auftraggeber =  leitkarte.get_prj_auftragsbez(); 
	auftraggeber.Remove('&');
	...
	"\n<center>\n<table border=\"1\" cellspacing=\"0\" cellpadding=\"3\" width=\"100%\">\n"
		...
		str = getStammdaten().getText("IDS_SACHBEARBEITER").c_str();
		out.Format("<br></br>\n</font></b><font face='arial' size='2'>%s:&#9;</font><b><font face='arial'>", str);
        head += out 
		...
		str = getStammdaten().getText("IDS_ORT").c_str();
		out.Format("<br></br>\n</font></b><font face='arial' size='2'>%s:&#9;</font><b><font face='arial'>", str); 
		CString temp = leitkarte.get_prj_bauOrt();
		temp.Remove('&');
		head += out
				+ temp
				+ "</font></b></td>\n</tr>\n";
		...
		str = getStammdaten().getText("IDS_LIEFERUNG_AM").c_str();
		out.Format("<font face='arial' size='2'><p align='center'>%s</p><br></br></font><b><font face='arial'>", str);
		...
		if(leitkarte.get_IsChoosenIppeNumbers())
		{
			...
			head +=	ippeMatNrstr
				+ " - " + sdpositionsnrstr + "<br>\n"  
				+ sdmaterialnrstr  + "<br></br>\n";
				
		}
		else
		{
			
			head += sdauftragsnrstr  + " - " + sdpositionsnrstr + "<br></br>\n"  
				+ sdmaterialnrstr  + "<br></br>\n";
		...
		if (leitkarte.get_prj_COauftrag().GetLength() > 1)
		{
			...
			out.Format("%s: ", str);
			head += out + leitkarte.get_prj_COauftrag() + "<br></br>\n";
		}
		else
			head += leitkarte.getPSPElement() + "<br></br>\n";
...
void DialogHtmlDruck::insert_bemerkung(const std::string& bem, const std::string& ueberschrift, const std::string& stdbemerkung)
{
	/**
...
void DialogHtmlDruck::insertListeUnterschriften()
{
	if (WaxSys::isAV())
	{
...
void DialogHtmlDruck::insertSondermuffentext(int laenge)
{
	if (laenge <= 0)
		return;
...
void DialogHtmlDruck::insertKFarbe(int kFarbe)
{
	if (!kFarbe)
		return;
	/*
...
void DialogHtmlDruck::insertEinzelpreise(CStringArray& preise)
{
...
void DialogHtmlDruck::insertQuali(int quali, bool twoquali, int dn, int stutzenAnlage2440, int anlagentyp )
{
	/*
	...
	*/
	...
	get_file().appendCol(tmp, 40, "center");
	get_file().endRow();
  	get_file().endTable();
}
...
void DialogHtmlDruck::insertLaengeGewichtBarcode(int anzahl, int maxlaenge, double gewicht, CString kc, int kcPosnr, string ids )
{
	/*
...
/*
	//if (gewicht > 0)
	{
		...
		strAnzahl = getStammdaten().getText("IDC_ANZAHL").c_str();
	}

	get_file().appendCol(strAnzahl, 100);
	strAnzahl.Format("%d", anzahl);
	get_file().appendCol("<b>" + strAnzahl + "</b>", 100, "center");
*/
	...
	get_file().appendCol(strGewicht, 150);
	get_file().appendCol("<b>" + sgewicht + "</b>", 100, "center");
...
void DialogHtmlDruck::insertTrennlinie()
{
...
void DialogHtmlDruck::insertSokoPreise(CObList &sokolist, double gesamtpreis)
{
	// Sokoliste
...
DialogWaxHtmlFile& DialogHtmlDruck::get_file()  
{
	return file;
}

void DialogHtmlDruck::insertVK3GesPreise(double elementepreis, double sonderkosten, double gesamtpreisVK3)
{
	// Gesamtpreise
...
void DialogHtmlDruck::insertVK3Preise(EnCObListVk3& vk3Liste, EnCObListVk3& passlist, EnCObListVk3& pulverlist, double elementepreis)
{
	// VK3 Elementliste
