...
#include "MehrfachDialogdrucklistenverwaltung.h"
...
int MehrfachDialogDrucklistenVerwaltung::verylastAusgabe = 0;

//currentAusgabe, ID der zu erzeugenden Ausgabe
int MehrfachDialogDrucklistenVerwaltung::currentAusgabe = 2;

MehrfachDialogDrucklistenVerwaltung::MehrfachDialogDrucklistenVerwaltung(MxCObList* pt, bool bWidthJScript) : waxpart(NULL), m_bWidthJScript(bWidthJScript)
{
	init(pt);
}

void MehrfachDialogDrucklistenVerwaltung::init(MxCObList* pt)
{
	...
	if (pt)
	{
		for (POSITION pos = pt->GetHeadPosition(); pos;)
		{
		
			walist.AddTail(pt->GetNext(pos));
		}
	}
}

//Destruktor
MehrfachDialogDrucklistenVerwaltung::~MehrfachDialogDrucklistenVerwaltung()
{
...
void MehrfachDialogDrucklistenVerwaltung::init_menu()
{
	//Liste mit Listenids (listen) und Listennamen (listennamen) initialisieren
	//listen enthält die für den jeweiligen Werkauftrag gültigen Drucklisten-IDs . 
	menuitems.clear();
...
#endif
		//Wenn Preisanfrage, dann keine weiteren Menüpunkte
		return;
	}
	...
	// Wenn Unteraufträge in der Liste sind, wird der letzte Auftrag benutzt.
	// Am Kopf könnte ein Sammelauftrag stehen, dessen Werkauftrag leer ist
...
string MehrfachDialogDrucklistenVerwaltung::get_name(int ausgabe_id, bool menu) const
{
	/**
	Liefert den Namen (für Menüeintrag bei menu = true oder Ausgabe) für eine spezifische Druckliste. 
	*/
...
currentAusgabe zurückgeliefert, sonst die ID der ersten Druckliste.
*/
bool MehrfachDialogDrucklistenVerwaltung::check_currentAusgabe() const
{
	/**
...
bool MehrfachDialogDrucklistenVerwaltung::set_default_druckliste()
{
	/**
	ausgaben Liste muss vorher gesetzt werden durch initmenuitems.
	Liefert false, wenn keine Druckliste ausgewählt werden kann.
	*/
	...
	// Kein vernünftiger Menüpunkt vorhanden, bzw. gar kein Menü (Projekt, Gliederung)
	currentAusgabe = DRUCKVORSCHAU_ABBRECHEN;
	return false;
}

//Erzeugt die gewünschte Ausgabedatei
const CString MehrfachDialogDrucklistenVerwaltung::makeAusgabe(int& iHowManyToPrint, CStringArray* aPath, int id)
{
	/*
	...
	*/
	...
	{
		 Werkauftrag* strang = dynamic_cast< Werkauftrag*>(walist.GetNext(pos));

	
		for(int h=0; h < aPath->GetSize(); h++)
		{
			CString strConc = aPath->GetAt(h);
			int iWo = strConc.Find('|');
			CString strName;
			strName = strConc.Left(iWo);
			CString strAnzahl;
			strAnzahl = strConc.Mid(iWo+1);
			int anzahl = atoi(strAnzahl);
			...
			for(int j=0; j < anzahl; j++)
			{
				...
				maxlaenge = strang->get_maxlaenge();
				gesamtgewicht = strang->getGesGewicht();
				
				write_header(strName, i);
				...
				html.insertLeitkarte( strang->get_leitkarte(), strang->get_status(), bez, strName, i);
				...
				{
					html.insertLaengeGewichtBarcode(strang->get_anzahl(), maxlaenge, gesamtgewicht, strang->get_leitkarte().get_kcAuftragsnr(), strang->get_leitkarte().get_kcPositionsnr(), "IDS_GESAMTGEWICHT_KG");
				}
				
				
				html.insertOberflaeche(strang, strang->get_beschichtung(), strang->get_zeugnis(), 
										   strang->get_sichtbereich(), strang->get_KollilisteExport());
				...
				if ((currentAusgabe == GRAPHIKDRUCK || currentAusgabe == STRANGROHRLISTE) && WaxSys::isAV())
				{
					...
					{
						if (strang->isEmpty())// || dynamic_cast<SammelWerkauftrag*>(cur_werkauftrag())) && (walist.GetCount() > 1))
							continue;
						if (strang->isGewinderohr() || dynamic_cast<FormstueckeFittinge*>(strang))
						{
							insert = false;
							break;
						}
					...
					if (insert)
					{
						...
						{
							html.insertZeiten(strang->get_arbeit().getZeitArbeitsplatz(Arbeitsplan::get_abpSchweissVorrichtplatz()), strang->get_arbeit().getZeitKostenstelle(k.getNr(Kostenstellen::BESCHICHTUNG)));
						}
					}
				}
				
				BOOL last = FALSE;
				if( i == walist.GetSize() * anzahl-1)
				{
					last = TRUE;
				}

				create_drucklist(strang, last);
...
/**
Erzeugt die gewählte Druckausgabe
*/
void MehrfachDialogDrucklistenVerwaltung::create_drucklist(Werkauftrag *ersterWerk, BOOL last)
{
	...
	*/	
	...
	//for (int i = 0; i < walist.GetCount(); i++, walist.AddTail(walist.GetHead()), walist.RemoveHead())
	{
		...
		if(dynamic_cast<SammelWerkauftrag*>(cur_werkauftrag()) && walist.GetCount() > 1)
			continue;
*/
		Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(ersterWerk);
		...
		if (first)
		{
			...
			{
				stdbemerkung = (LPCSTR)cur_werkauftrag()->get_stdbemerkung() ;
			}
			if(m_bWidthJScript)
			{
		...
		switch (currentAusgabe)
		{
			case GRAPHIKDRUCK:
			{			
				createGraphikdruck(ersterWerk, last); 
			}
			break;
			case ELEMENTPLAN:
				createElementplan(); 
		...
		/*
		// nicht beim ersten und letzten Werkauftrag einfügen
	...
	// Ausgabe von Einzellisten generiert aus temporären Daten die in initListen initialisiert wurden
	switch (currentAusgabe)
	{
...
/**
An diese Ausgabe wird der Rest der Druckliste angehängt.
*/
void MehrfachDialogDrucklistenVerwaltung::write_header(CString name, int i)
{
...
//Druckliste nicht verfügbar ist, bzw. wenn keine Druckliste verfügbar ist.
void MehrfachDialogDrucklistenVerwaltung::createDefaultDruckliste( )
{
	/*
...
bool MehrfachDialogDrucklistenVerwaltung::insertGrafic(Werkauftrag* werk, BOOL last)
{
	/*
	Grafikdatei erzeugen und in Html-Datei einfügen
	*/
	Rohrleitung *rohr = dynamic_cast<Rohrleitung*>(werk);
	...
	{
		if (!rohr->get_anhang()->get_data())
			return false;

		//Endung ermitteln
		CString end = rohr->get_anhang()->get_filename().Right(3);
		...
		if (tmpfile.is_open())
			tmpfile.write((const char*)rohr->get_anhang()->get_data(), rohr->get_anhang()->get_len());
		...
		{
			*/
			html.get_file().DialogWaxHtmlFile::insertDWFLink(fn, end);
	...
	if (WaxSys::isAV() && !cur_werkauftrag()->get_posNr().IsEmpty())
	{
		...
		{
					CString strAnzahlFirst = getStammdaten().getText("IDC_ANZAHL").c_str();
	

	CString strAnzahl;
	strAnzahl.Format("%d", rohr->get_anzahl());
	
	DialogWaxHtmlFile&  w = html.get_file();
	w.startTable(0,0);
	w.startRow();
	
	w.appendCol( "<b><font face='Arial' >" + strAnzahlFirst + " </font></b>",	15, "left", true);
	w.appendCol("<b><font face='Arial' >" +  strAnzahl + " </font></b>",		85, "left", true);
	
	w.endRow();
	w.endTable();
	html.get_file().WriteString(html.get_file().createBarcode(100, 20, rohr->get_posNr().GetString(), "left"));
	if(last == FALSE)
	{
		html.get_file().insertSeitenumbruch();
	}
		}
    ...
    {
		JScriptVisualisationAndPrinting.createBlankJPG(rohr);
	}
...
void MehrfachDialogDrucklistenVerwaltung::createVK3Abrechnungdata()
{
...
void MehrfachDialogDrucklistenVerwaltung::createAbrechnung()
{
	if (!WaxSys::isAV())
		html.insertEinzelpreise(preise);  
}

void MehrfachDialogDrucklistenVerwaltung::createVK3Abrechnung()
{
	if (!WaxSys::isAV())
		html.insertEinzelpreise(preise);  
...
void MehrfachDialogDrucklistenVerwaltung::createGraphikdruck(Werkauftrag* Werkrohr, BOOL last)
{
	/*
	...
	*/
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(Werkrohr);
	ApaxWerkauftrag* apax = dynamic_cast<ApaxWerkauftrag*>(cur_werkauftrag());
	if (!rohr && !apax)
	{
		createDefaultDruckliste();
...
void MehrfachDialogDrucklistenVerwaltung::createElementplan()
{
// Die Elementdaten werden an mehreren Stellen in die Liste eingefügt. Statt den Code zu kopieren,
// gibt es auch die Möglichkeit ein Makro zu definieren. 
// Unterschiedlich beim Einfügen ist nur der Marker.
...
//Druckliste für den Rohrleitungsplan, die Elemente der Rohrkonstruktion 
//werden hier direkt ausgegeben
void MehrfachDialogDrucklistenVerwaltung::createRohrleitungsplan()
{
	/**
	Aufgrund des Wunsches eines einzelnen Herrn an der Maschine wird bei Muffen grundsätzlich
...
w.appendCol(element->get_abstAbs(), rows[6]); }
	...
	for (POSITION pos = elementList.GetHeadPosition(); pos != NULL;)	
	{
		...
		if (pos)
		{ // Wenn das Folgeelement ein Rohr ist, wird für dieses Rohr eine neue Tabelle angefangen
		...
		if (element->get_id() != Element::LEER)
		{
									  ...
									  && eleID != ERED	&& eleID != KRED)
			{
				...
				//e.set_abstRel(0); // Weil Werte der absolute Abstand des Elements bezogen auf das Rohrstueck ist und nicht
								  ...
								  // Zum Verständnis diesen Part kommentieren und Rohr mit Red. eingeben!   	
			}
			INSERT_ELEMENT((&e), "");
		}
...
void MehrfachDialogDrucklistenVerwaltung::createSaegenutanlageliste( )
{
	...
	for (int count=0; count<elelst.get_count(); count++)	
	{
		...
		if (element->isEleRohr())
		{
			...
			if (element->getLaengeZuschnitt() > 1499 && element->get_dn() > 50 && element->get_dn() < 250) 
			{
				...
				{
					// Überschrift für die Unterelementliste
					w.startRow();
					str = getStammdaten().getText("IDS_ELEMENTTYP").c_str();
					...
					if (drehen)
					{
						for (POSITION upos = uelementList.GetTailPosition(); upos != NULL;)	
						{
															 ...
															 || uelement->get_id() == BOHR50) && ( strangrohr && strangrohr->get_dnvl() == 65 && strangrohr->get_muffenDN() == 25)))
							{
								...
								// sonst bleibt vom Rohr nichts mehr übrig. Das zweite wird von Hand gemacht.
...
void MehrfachDialogDrucklistenVerwaltung::Saegenutanlageliste_insert_uelement(WaxHtmlFile &w, const Element *uelement, const int eleAbst, const int eleWinkel)
{
...
void MehrfachDialogDrucklistenVerwaltung::createGewinderohrdata()
{
...
//Druckliste für die Pumpenprobierleitung. Die Grafik wird als Bitmap 
//eingebunden, wie Sondermz. Grafik ist VDS abgenommen. Für Infos: Hr. 
//Richter
void MehrfachDialogDrucklistenVerwaltung::createPumpenprobierleitung( )
{
...
//Druckliste für den Pumpenverteiler. Die Grafik wird als Bitmap 
//eingebunden, wie Sondermz. Grafik ist VDS abgenommen. Für Infos: Hr. 
//Richter
void MehrfachDialogDrucklistenVerwaltung::createPumpenverteiler( )
{
	...
	// Graphik Originalgrösse ist 252x201. Hier um 25% aufgezoomt
	w.WriteString("<table width='100%' border='1' cellpadding='2' cellspacing='0'>");
	w.WriteString("<tr><TD WIDTH='100%' VALIGN='MIDDLE' COLSPAN=1>\n"
				"<P ALIGN='CENTER'><IMG SRC='pumpenverteiler.jpg' WIDTH=375 HEIGHT=251 ></TD>\n</tr>");
}

//Druckliste für die EntnahmeleitungDWB. Die Grafik wird als Bitmap 
//eingebunden, wie Sondermz. Grafik ist VDS abgenommen. Für Infos: Hr. 
//Richter
void MehrfachDialogDrucklistenVerwaltung::createEntnahmeleitungDWB( )
{
...
void MehrfachDialogDrucklistenVerwaltung::createProfilliste(int liste)
{
	#define SEITENANSICHT -1
	
	// Tabellenspalten
	const int titel[] = { 30, 30, 270, 30, 300, 30 }; 
	const int rows2[] = {    330, 30, 300, 30 }; // Zeile mit Winkel und Länge
	...
	for (POSITION pos = proflist.GetHeadPosition(); pos; )
	{
		...
		// Zeile mit Winkel und Längenangaben
		w.startRow();
		i = 0;
		CString fmt;
...
bool MehrfachDialogDrucklistenVerwaltung::check2Rohrquali(const CObList& walist) const
{
	/**
...
void MehrfachDialogDrucklistenVerwaltung::set_werkauftrag(Werkauftrag* w)
{
	if (w)
		walist.AddHead(w);
	werkauftrag = w;
...
//set walist
void MehrfachDialogDrucklistenVerwaltung::set_walist(CObList& value)
{
	/*
...
void MehrfachDialogDrucklistenVerwaltung::set_stklist(const CObList& value, bool nullpreis, bool einkauf)
{
	int test = 0;
...
void MehrfachDialogDrucklistenVerwaltung::setGesamtmaterialliste()
{
	// Stueckliste vom Werkauftrag scannen
...
void MehrfachDialogDrucklistenVerwaltung::set_matlist(const CObList& value, int list, bool nullpreis, bool einkauf)
{
	...
	for (POSITION pos = value.GetHeadPosition(); pos;)
	{
		...
		if (einkauf || (!einkauf && !mat->get_einkauf()))
		{
			...
			if ((list == MehrfachDialogDrucklistenVerwaltung::MATERIALLISTE_GESAMT)
				|| (list == MehrfachDialogDrucklistenVerwaltung::STRANGROHRLISTE))
			{
				// Nur Material ohne Preis?
...
void MehrfachDialogDrucklistenVerwaltung::set_sokolist(const CObList& value)
{
...
void MehrfachDialogDrucklistenVerwaltung::set_passlist(const CObList& value)
{
...
void MehrfachDialogDrucklistenVerwaltung::set_pulverlist(EnCObListVk3& value, bool sort)
{
...
//Liste kopieren und wenn sort = true, dann kumulieren
void MehrfachDialogDrucklistenVerwaltung::set_elelist(EnCObListVk3& value, bool sort)
{
	EnCObListVk3 tmpListe;
...
void MehrfachDialogDrucklistenVerwaltung::check_NullEuro()
{
	CString nulleuro;
...
void MehrfachDialogDrucklistenVerwaltung::reset()
{
	anzahlRohre = 0;
	anzahlElemente = 0;
	maxlaenge = 0;
...
void MehrfachDialogDrucklistenVerwaltung::setGesamtpreise()
{
...
void MehrfachDialogDrucklistenVerwaltung::setMaxLaengeUndGewicht()
{
...
void MehrfachDialogDrucklistenVerwaltung::setVK3Gesamtpreis()
{
...
//get currentAusgabe
const int MehrfachDialogDrucklistenVerwaltung::get_currentAusgabe() const
{
...
//set currentAusgabe
void MehrfachDialogDrucklistenVerwaltung::set_currentAusgabe(int value)
{
	currentAusgabe = value;
}

// get lastAusgabe
int MehrfachDialogDrucklistenVerwaltung::get_lastAusgabe() const
{
	return verylastAusgabe;
}

//set lastAusgabe
void MehrfachDialogDrucklistenVerwaltung::set_lastAusgabe(int value)
{
	verylastAusgabe = value;
}

const vector<int>& MehrfachDialogDrucklistenVerwaltung::get_menuitems() const
{
	return menuitems;
}

void MehrfachDialogDrucklistenVerwaltung::set_ziel(const CString& z)
{
	ziel = z;
}


void MehrfachDialogDrucklistenVerwaltung::set_dn2zoll(int menuitem)
{
	switch (menuitem)
	{
...
//get werkauftrag
Werkauftrag* MehrfachDialogDrucklistenVerwaltung::cur_werkauftrag() 
{
...
vector<Werkauftrag*> MehrfachDialogDrucklistenVerwaltung::get_walist()
{
...
bool MehrfachDialogDrucklistenVerwaltung::init_material_menu() 
{
	...
	for (vector<Werkauftrag*>::iterator it = walst.begin(); it != walst.end(); it++)
	{
		...
		// Materialbestellungen Listen werden nur eingefügt, wenn auch wirklich Material und 
