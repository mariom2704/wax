...
DrucklisteWap::DrucklisteWap(HtmlDruck& file, WaxPart* pt, GpWapPreis::PREISART part) : Druckliste(file, pt)
{
	preisart = part;
}

DrucklisteWap::~DrucklisteWap()
{
}


DrucklisteWap::DrucklisteWap(HtmlDruck& file, CObList* ptWerkauf, GpWapPreis::PREISART part) : Druckliste(file, ptWerkauf)	
{
	preisart = part;
}
...
*/
double DrucklisteWap::createWap()
{
	addBemerkung();
	...
	addFertigungskosten(gesFert, true, ueberschrift);
...
int DrucklisteWap::get_AnzahlRohreProPos()
{
...
*/
bool DrucklisteWap::createMaterialliste(bool showPreise)
{
	addBemerkung();
	addRohrQuali();
...
bool DrucklisteWap::createArbeitsplan(bool showPreise)
{
	addBemerkung();
	addRohrQuali();

	double gesFert = 0;
	addFertigungskosten(gesFert, showPreise, "");
...
void DrucklisteWap::addMaterialkosten(double &gesMat, bool showPreis, const string ueberschrift)
{
	//Gesamtliste bilden
...
void DrucklisteWap::addFertigungskosten(double &gesPreisMitZuschlag, bool showPreise, const string ueberschrift)
{
...
void  DrucklisteWap::setGesamtArbeitsplan(Arbeitsplan &ap, Beschichtung &beschichtung, int &maxLaenge, double &gesGewicht, int &wanr)
{
	gesGewicht = 0;
	maxLaenge = 0;
...
void DrucklisteWap::setGesamtmaterialliste(MaterialListe &matliste)
{
	bool isSammel = false;
...
void DrucklisteWap::addGesKosten(double gesMat, double gesFert, double gesFremd, double &selbstkosten)
{
	WaxHtmlFile &w = html.get_file();
...
void DrucklisteWap::addFremdleistungen(double &gesFremd)
{
	gesFremd = 0;
...
void DrucklisteWap::getZinkkosten(ZinkKosten &zk)
{
	for (Auftragsliste::iterator it = auftraege.begin(); it != auftraege.end(); it++)
	{		
...
void DrucklisteWap::addRohrQuali()
{
	//Rohr suchen
	for (Auftragsliste::iterator it = auftraege.begin(); it != auftraege.end(); it++)
	{		
...
void DrucklisteWap::addBemerkung()
{
...
void DrucklisteWap::addVk3(double vk3Ges, double selbstkosten)
{
