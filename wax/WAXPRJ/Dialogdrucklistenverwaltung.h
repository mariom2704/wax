...
namespace WaxDruck
{
	...
	*/
	class WAXPRJDLL DialogDrucklistenVerwaltung
	{
		Maschinenlisten* maschlist;
	/// public
	public:
		//CJScriptPrinting JScriptVisualisationAndPrinting;

		void SetMaschliste(Maschinenlisten* maschlistLocal)
		{
			maschlist = maschlistLocal;
		}
		...
		DialogDrucklistenVerwaltung(int i, MxCObList* pt, bool bWidthJScript= false);

		DialogDrucklistenVerwaltung(){init(NULL);};

		/// Destruktor
		virtual ~DialogDrucklistenVerwaltung();

		/// Erzeugt die gewünschte Ausgabedatei
		const CString makeAusgabe(int& iHowManyToPrint, int id = 0);
	...
	private:
		...
		void create_drucklist(Werkauftrag *ersterWerk, BOOL last);
		...
		bool insertGrafic(Werkauftrag* werk, BOOL last);

		/// Erzeugt den Graphikdruck
		void createGraphikdruck(Werkauftrag* rohr, BOOL last);
