...
//{
	...
	class WAXPRJDLL DialogHtmlDruck 
	{
	public:
		...
		DialogHtmlDruck(int i);

		/// Zugriff auf die Basisfunktionen der Html-Ausgabe
		 DialogWaxHtmlFile& get_file()  ;

		virtual ~DialogHtmlDruck();

		DialogHtmlDruck(CString fn);
		
		/// Fügt eine Zeile mit der Beschichtung, dem Sichtbereichsflag, dem 
		/// Zeugnisflag und der Zulassungsstelle ein
		void insertOberflaeche(Werkauftrag* werkauft, const Beschichtung &beschichtung, BOOL zeugnis, BOOL sichtbereich, int zulassungsstelle);
		...
		void insertLeitkarte( Leitkarte& leitkarte, Status& status, const CString& name, const CString & ziel, int i, bool umbruch = false);

		void WriteCuttingHeader(CString Name);
		...
		void insertLaengeGewichtBarcode(int anzahl, int maxlaenge, double gewicht, CString kc, int kcPosnr,  string ids );
