...
#include "Dialogdrucklistenverwaltung.h"
...
int DialogDrucklistenVerwaltung::verylastAusgabe = 0;

//currentAusgabe, ID der zu erzeugenden Ausgabe
int DialogDrucklistenVerwaltung::currentAusgabe;

DialogDrucklistenVerwaltung::DialogDrucklistenVerwaltung(int sys, MxCObList* pt, bool bWidthJScript) : waxpart(NULL), m_bWidthJScript(bWidthJScript), html(sys)
{
	currentAusgabe = sys;
	init(pt);
}

void DialogDrucklistenVerwaltung::init(MxCObList* pt)
{
...
//Destruktor
DialogDrucklistenVerwaltung::~DialogDrucklistenVerwaltung()
{
...
void DialogDrucklistenVerwaltung::init_menu()
{
...
string DialogDrucklistenVerwaltung::get_name(int ausgabe_id, bool menu) const
{
	/**
...
*/
bool DialogDrucklistenVerwaltung::check_currentAusgabe() const
{
	/**
...
bool DialogDrucklistenVerwaltung::set_default_druckliste()
{
	/**
...
const CString DialogDrucklistenVerwaltung::makeAusgabe(int& iHowManyToPrint, int id)
{
	/*
	...
	*/
	int i = 0;
	if (currentAusgabe != GRAPHIKDRUCK && currentAusgabe != ARBEITSPLAN)
	{
		create_drucklist(NULL, FALSE);
	}
	else
	{
	...
	html.insertLeitkarte(strang->get_leitkarte(), strang->get_status(), bez, ziel, i);
...
*/
void DialogDrucklistenVerwaltung::create_drucklist(Werkauftrag* werkauftragLocal, BOOL last)
{
		...
		case ZUSCHNITTSLISTE_38030_ANLAGE:
			{
				...
				CString bez = get_name(currentAusgabe, false).c_str();
				html.WriteCuttingHeader(bez);
				DialogDrucklisteZuschnitt z(html, NULL, currentAusgabe);
				z.SetMaschListe(maschlist);
				z.create(werkauftragLocal);
			}
			break;
			...
			{
				//CObList werkauftrag;
				//werkauftrag.AddTail(werkauftragLocal);
				for (POSITION pos = walist.GetHeadPosition(); pos != NULL; )
				{
					CObject* pwerkauftrag = dynamic_cast< CObject*>(walist.GetNext(pos));
					htmlFiles = html.get_file().GetFilePath();
					CObList werkauftrag;
					werkauftrag.AddTail(pwerkauftrag);
					DialogDrucklisteWap wap(html, &werkauftrag, GpWapPreis::Vollkosten);
...
/*
	...
	*/
	if (werkauftragLocal)
		bemerkung = (LPCSTR)werkauftragLocal->get_bemerkung();
	...
	*/	
	...
	//for (int i = 0; i < walist.GetCount(); i++, walist.AddTail(walist.GetHead()), walist.RemoveHead())
	{
		...
		if(dynamic_cast<SammelWerkauftrag*>(cur_werkauftrag()) && walist.GetCount() > 1)
			continue;
*/
		Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(werkauftragLocal);
		...
		switch (currentAusgabe)
		{
			case GRAPHIKDRUCK:
			{			
				createGraphikdruck(werkauftragLocal, last); 
			}
			break;
			case ARBEITSPLAN:
			{
				
				CObject* pwerkauftrag = werkauftragLocal;
				htmlFiles = html.get_file().GetFilePath();
				CObList werkauftrag;
				werkauftrag.AddTail(pwerkauftrag);
				DialogDrucklisteWap wap(html, &werkauftrag, GpWapPreis::Vollkosten);
				wap.createArbeitsplan(false, last);
			}
			break;
...
*/
void DialogDrucklistenVerwaltung::write_header()
{
...
void DialogDrucklistenVerwaltung::createDefaultDruckliste( )
{
	/*
...
bool DialogDrucklistenVerwaltung::insertGrafic(Werkauftrag* werk, BOOL last)
{
	/*
	...
	*/
	...
	{
		//Barcode für Positionsnummer erzeugen
...
void DialogDrucklistenVerwaltung::createVK3Abrechnungdata()
{
...
void DialogDrucklistenVerwaltung::createAbrechnung()
{
	if (!WaxSys::isAV())
		html.insertEinzelpreise(preise);  
}

void DialogDrucklistenVerwaltung::createVK3Abrechnung()
{
	if (!WaxSys::isAV())
		html.insertEinzelpreise(preise);  
...
void DialogDrucklistenVerwaltung::createGraphikdruck(Werkauftrag* Werkrohr, BOOL last)
{
	/*
...
void DialogDrucklistenVerwaltung::createElementplan()
{
...
void DialogDrucklistenVerwaltung::createRohrleitungsplan()
{
	/**
...
void DialogDrucklistenVerwaltung::createSaegenutanlageliste( )
{
...
void DialogDrucklistenVerwaltung::Saegenutanlageliste_insert_uelement(WaxHtmlFile &w, const Element *uelement, const int eleAbst, const int eleWinkel)
{
...
void DialogDrucklistenVerwaltung::createGewinderohrdata()
{
...
//Richter
void DialogDrucklistenVerwaltung::createPumpenprobierleitung( )
{
...
//eingebunden, wie Sondermz. Grafik ist VDS abgenommen. Für Infos: Hr. 
//Richter
void DialogDrucklistenVerwaltung::createPumpenverteiler( )
{
...
//eingebunden, wie Sondermz. Grafik ist VDS abgenommen. Für Infos: Hr. 
//Richter
void DialogDrucklistenVerwaltung::createEntnahmeleitungDWB( )
{
...
void DialogDrucklistenVerwaltung::createProfilliste(int liste)
{
...
bool DialogDrucklistenVerwaltung::check2Rohrquali(const CObList& walist) const
{
	/**
...
void DialogDrucklistenVerwaltung::set_werkauftrag(Werkauftrag* w)
{
	if (w)
		walist.AddHead(w);
	werkauftrag = w;
...
//set walist
void DialogDrucklistenVerwaltung::set_walist(CObList& value)
{
	/*
...
void DialogDrucklistenVerwaltung::set_stklist(const CObList& value, bool nullpreis, bool einkauf)
{
	int test = 0;
...
void DialogDrucklistenVerwaltung::setGesamtmaterialliste()
{
	// Stueckliste vom Werkauftrag scannen
...
void DialogDrucklistenVerwaltung::set_matlist(const CObList& value, int list, bool nullpreis, bool einkauf)
{
	...
	for (POSITION pos = value.GetHeadPosition(); pos;)
	{
		...
		if (einkauf || (!einkauf && !mat->get_einkauf()))
		{
			...
			if ((list == DialogDrucklistenVerwaltung::MATERIALLISTE_GESAMT)
				|| (list == DialogDrucklistenVerwaltung::STRANGROHRLISTE))
			{
				// Nur Material ohne Preis?
...
void DialogDrucklistenVerwaltung::set_sokolist(const CObList& value)
{
...
void DialogDrucklistenVerwaltung::set_passlist(const CObList& value)
{
...
void DialogDrucklistenVerwaltung::set_pulverlist(EnCObListVk3& value, bool sort)
{
...
//Liste kopieren und wenn sort = true, dann kumulieren
void DialogDrucklistenVerwaltung::set_elelist(EnCObListVk3& value, bool sort)
{
	EnCObListVk3 tmpListe;
...
void DialogDrucklistenVerwaltung::check_NullEuro()
{
	CString nulleuro;
...
void DialogDrucklistenVerwaltung::reset()
{
	anzahlRohre = 0;
	anzahlElemente = 0;
	maxlaenge = 0;
...
void DialogDrucklistenVerwaltung::setGesamtpreise()
{
...
void DialogDrucklistenVerwaltung::setMaxLaengeUndGewicht()
{
...
void DialogDrucklistenVerwaltung::setVK3Gesamtpreis()
{
...
//get currentAusgabe
const int DialogDrucklistenVerwaltung::get_currentAusgabe() const
{
...
//set currentAusgabe
void DialogDrucklistenVerwaltung::set_currentAusgabe(int value)
{
	currentAusgabe = value;
}

// get lastAusgabe
int DialogDrucklistenVerwaltung::get_lastAusgabe() const
{
	return verylastAusgabe;
}

//set lastAusgabe
void DialogDrucklistenVerwaltung::set_lastAusgabe(int value)
{
	verylastAusgabe = value;
}

const vector<int>& DialogDrucklistenVerwaltung::get_menuitems() const
{
	return menuitems;
}

void DialogDrucklistenVerwaltung::set_ziel(const CString& z)
{
	ziel = z;
}


void DialogDrucklistenVerwaltung::set_dn2zoll(int menuitem)
{
	switch (menuitem)
	{
...
//get werkauftrag
Werkauftrag* DialogDrucklistenVerwaltung::cur_werkauftrag() 
{
...
vector<Werkauftrag*> DialogDrucklistenVerwaltung::get_walist()
{
...
bool DialogDrucklistenVerwaltung::init_material_menu() 
{
