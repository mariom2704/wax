// waxprj.cpp : Legt die Initialisierungsroutinen für die DLL fest.
//
...
static AFX_EXTENSION_MODULE waxprjDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("WAXPRJ.DLL Initializing!");
		TRACE(" hInstance = 0X%x\n", hInstance);
		
		// One-Time-Initialisierung der Erweiterungs-DLL
		if (!AfxInitExtensionModule(waxprjDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(waxprjDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("WAXPRJ.DLL Terminating!\n");
		// Bibliothek vor dem Aufruf der Destruktoren schließen
		AfxTermExtensionModule(waxprjDLL);
	}
	return 1;   // OK
}
