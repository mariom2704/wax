...
DrucklisteZuschnitt::DrucklisteZuschnitt(HtmlDruck& file, WaxPart* pt, int id) 
: Druckliste(file, pt), maschinenlisten(pt)
{
	schweissung = both;
	listid = id;
...
DrucklisteZuschnitt::~DrucklisteZuschnitt()
{
	delete default_rohr;
}

CObList* DrucklisteZuschnitt::listefromid(int id)
{
	/**
...
void DrucklisteZuschnitt::add_drucklisten(vector<int>& menuitems) const
{
...
bool DrucklisteZuschnitt::create()
{
	...
	*/
	// Tabellenspalten	
	const int rows[] = { 35, 75, 180, /*80,*/ 80, 75, 60, 60, 130};
	...
	for (POSITION p = list.GetHeadPosition(); p != NULL;)	
	{
		...
		//Positionsnr.
		w.appendCol(element->get_rohrPosNr(),rows[spalte++], "LEFT");
...
Rohrleitung* DrucklisteZuschnitt::scan_rohr(bool& mehrereRohrquali)
{
	/**
