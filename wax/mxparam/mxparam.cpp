...
static AFX_EXTENSION_MODULE MxParamDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		//TRACE0("MXPARAM.DLL Initializing!");
		// One-Time-Initialisierung der Erweiterungs-DLL
		if (!AfxInitExtensionModule(MxParamDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MxParamDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		//TRACE0("MXPARAM.DLL Terminating!\n");
		// Bibliothek vor dem Aufruf der Destruktoren schließen
		AfxTermExtensionModule(MxParamDLL);
	}
	return 1;   // OK
}
