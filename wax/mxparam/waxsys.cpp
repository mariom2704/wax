...
{
	/**
	Öffnet die Benutzerverwaltung zur Eingabe der persönlichen Daten. Ist detect_new_user
...
MXPARAMDLL void WaxSys::remove_tmpfiles()
{
	...
	{
		TRACE("Temporäre Datei %s wird gelöscht\n", it->c_str());
		...
		{
			TRACE("%s konnte nicht gelöscht werden: GetLastError=%i\n", it->c_str(), GetLastError());
			it++;
		}
	}
#ifdef _DEBUG
...
//Liefert den Connectstring für die Userdatenbank
...
{
	/**
	Prüft max. zwei Tasten gleichzeitig. Wird nur key1 benötigt, muss für key2 Null übergeben werden (default)
	*/
