// waxint.cpp : Legt die Initialisierungsroutinen für die DLL fest.
//
...
static AFX_EXTENSION_MODULE WaxintDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("WAXINT.DLL Initializing!\n");
		
		// One-Time-Initialisierung der Erweiterungs-DLL
		if (!AfxInitExtensionModule(WaxintDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(WaxintDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("WAXINT.DLL Terminating!\n");
		// Bibliothek vor dem Aufruf der Destruktoren schließen
		AfxTermExtensionModule(WaxintDLL);
	}
	return 1;   // OK
}
