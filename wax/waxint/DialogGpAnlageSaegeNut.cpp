...
#include "DialogGpAnlageSaegeNut.h"
#include "..\rohrkonst\mxcoblist.h"
#include "resource.h"

DialogGpAnlageSaegeNut::DialogGpAnlageSaegeNut(MxCObList *auftraege)
...
DialogGpAnlageSaegeNut::~DialogGpAnlageSaegeNut(void)
{
}
...
Tabellen schreiben
*/
bool DialogGpAnlageSaegeNut::createSaegeNutVerteilerDb()
{
...
Werkauftrag *DialogGpAnlageSaegeNut::getFirstAuftrag()
{
	MxCObList *auftraege = get_auftraege();
	if (!auftraege)
		return NULL;
...
bool DialogGpAnlageSaegeNut::createWerte(Werkauftrag *werkauftrag)
{
	...
	{
		if (iterSnR->typ == (int)DialogGpAnlageSaegeNut::TypVerteilerAnlage)
		{
			kopfData.anlagentyp = (int)DialogGpAnlageSaegeNut::AnlageSaegeNutUndVerteiler;
			break;
		}
		iterSnR++;
	}
	...
	saegeNutRohrListe.sort(DialogGpAnlageSaegeNut::greater);
	saegeNutRohrListeLegacy.sort(DialogGpAnlageSaegeNut::greater);
...
*/
bool DialogGpAnlageSaegeNut::cleanTables(int wanr)
{
...
*/
bool DialogGpAnlageSaegeNut::cleanTablesWaid(int waid)
{
...
*/
bool DialogGpAnlageSaegeNut::createRohre(Werkauftrag *werkauftrag)
{
   ...
   //Rohrtyp
	...
	if (handelsl)
		rohrDaten.typ = (int)DialogGpAnlageSaegeNut::TypHandelslaenge;
	else
		rohrDaten.typ = (int)DialogGpAnlageSaegeNut::TypVerteilerhandschweissung;
	...
	{	
		...
		if (rohrDaten.typ != (int)DialogGpAnlageSaegeNut::TypHandelslaenge && unterElemente.hasAnlagenTeile() && rohrDaten.dn <= maxDnVerteiler)
			rohrDaten.typ = (int)DialogGpAnlageSaegeNut::TypVerteilerAnlage;
		else
			rohrDaten.typ = (int)DialogGpAnlageSaegeNut::TypVerteilerhandschweissung;

		//Ablageplatz ermitteln
		if (rohrDaten.typ == (int)DialogGpAnlageSaegeNut::TypHandelslaenge)
			rohrDaten.ablageplatz = DialogGpAnlageSaegeNut::AblageHandelslaenge;
		else if (rohrDaten.typ == (int)DialogGpAnlageSaegeNut::TypVerteilerAnlage)
			rohrDaten.ablageplatz = DialogGpAnlageSaegeNut::AblageVerteilerAnlage;
		else if (rohrDaten.laenge <= maxLaengeAblage4)
			rohrDaten.ablageplatz = DialogGpAnlageSaegeNut::AblageVerteilerHand4;
		else
			rohrDaten.ablageplatz = DialogGpAnlageSaegeNut::AblageVerteilerHand3;
		...
		bool drehen = false;
		if (rohrDaten.typ == (int)DialogGpAnlageSaegeNut::TypVerteilerAnlage)
		{
...
*/
bool DialogGpAnlageSaegeNut::greater(DbAnlage::saegeNutRohre r1, DbAnlage::saegeNutRohre r2)
{
...
*/
bool DialogGpAnlageSaegeNut::writeDatenInDb()
{
	//Transaktion
...
*/
bool DialogGpAnlageSaegeNut::createLoecher(int absAbstandRohr, bool drehen, 
									...
									int &lochCounter)
{
	...
	{	
		...
		if (element->get_id() == MUFFE || element->get_id() == SONDERMUFFE || element->get_id() == STDMUFFE)
			lochDaten.lochTyp = (int)DialogGpAnlageSaegeNut::Muffe;
		else if (element->get_id() == NUTST || element->get_id() == STDSTUTZEN || element->get_id() == GWRST)
			lochDaten.lochTyp = (int)DialogGpAnlageSaegeNut::Nutstutzen;
		else if (element->get_id() == ROHR)
			lochDaten.lochTyp = (int)DialogGpAnlageSaegeNut::Rohr;
		else if (element->get_id() == BOHR50)
			lochDaten.lochTyp = (int)DialogGpAnlageSaegeNut::Bohrung;
		else
			continue;
...
bool DialogGpAnlageSaegeNut::createKopfsatz(Werkauftrag *werkauftrag)
{
	//Kopfsatz schreiben
	...
	kopfData.anlagentyp = (int)DialogGpAnlageSaegeNut::AnlageSaegeNut;
...
*/
bool DialogGpAnlageSaegeNut::checkStatus(int wanr)
{
...
bool DialogGpAnlageSaegeNut::createVerteilerRohre(Rohrleitung *rohr, DbAnlage::verteilerRohre &vertRohrDaten)
{
...
void DialogGpAnlageSaegeNut::changeBoolValue(bool &wert1, bool &wert2)
{
	bool tmp = wert1;
	wert1 = wert2;
	wert2 = tmp;
...
bool DialogGpAnlageSaegeNut::readAuftraege(GpAnlage::Status status, list<DbAnlage::kopfsatz> &data, MXDate date)
{	
