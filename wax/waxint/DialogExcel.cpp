...
#include "DialogGpAnlageVerteiler.h"
...
DialogExcel::DialogExcel(Werkauftrag* wa) : werkauftrag(wa), currentAction(0)
{ 
}



void DialogExcel::setExcelTableOfficeVersion2010(bool bOfficeVersion)
{
...
const CWordArray& DialogExcel::get_aktionen()
{
	return aktionen;
}

void DialogExcel::set_currentAction(int value)
{
	currentAction = value;
}

Werkauftrag* DialogExcel::get_werkauftrag()
{
...
void DialogExcel::set_werkauftrag(Werkauftrag* value)
{
	werkauftrag = value;
...
CString DialogExcel::getName(int id)
{
/*
...
void DialogExcel::sortWerkauftraege(int actionid)
{
...
void DialogExcel::makeAction(int id)
{
/*
...
*/
	...
	DialogGpAnlageSaegeNut saegeNut(&walist);
	DialogGpAnlageVerteiler verteiler(&walist);
...
*/
bool DialogExcel::createZusammenfassungAuftraegeRobbi()
{
	BefehlZusammenfassungWerkauftraegeRobbi befehl;
	return befehl.start();
}

int DialogExcel::createAuftragsdata()
{
	...
	if (neuwa)
	{
		...
		if (grohr)
		{
			...
			zeitSegment.Add(get_werkauftrag()->get_arbeit().getZeitArbeitsplatz(Arbeitsplan::get_abpSchweissVorrichtplatz()));
...
/*
Pakt Auftragsdaten in eine DialogExceldatei. 
*/
CString DialogExcel::createAuftragstabelle(CString fname)
{
	...
	CString datum = MXDate::GetCurrentTime().Format("%d%m%y_%H%M%S");
	fn += "WAX Aufträge ";
	fn += datum;
	fn += ".xls";
	...
	for (int i=0; i<wan.GetSize(); i++)
	{
		...
		CString txt7, txt8, txt9, txt10, txt11, txt12, txt13;
		...
		strLaenge.Format("%d", rohrlaengen.GetAt(i)/1000);
...
int DialogExcel::createSaegenutdata()
{
...
void DialogExcel::createLeitkartedata()
{
	...
	if (neuwa)
	{
		...
		strKolliliste.Format("%s",   (get_werkauftrag()->get_leitkarte().get_zulassungsnummer() == 2) ? "FM" : "VdS");
...
void DialogExcel::createRohrdata(int maxAnzStdele)
{
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(get_werkauftrag());
	XASSERT(rohr);
	if (!rohr) 
...
void DialogExcel::createRohrdataStdvert()
{
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(get_werkauftrag());
	XASSERT(rohr);
	if (!rohr) 
...
void DialogExcel::createRohrdataStrang(int maxAnzStdele)
{
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(get_werkauftrag());
	XASSERT(rohr);
	if (!rohr) 
...
bool DialogExcel::createVerteilerdata()
{
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(get_werkauftrag());
	if (!rohr->isVerteiler() || !rohr->checkAnlage()) 
		return true;
...
bool DialogExcel::setTableLeitkarte(char fname[], bool createTable)
{
	if (createTable)
	{
		// Table erstellen
...
bool DialogExcel::setTableVerteilerdaten(int index, char fname[], bool createTable)
{
	Rohrleitung* rohr = dynamic_cast<Rohrleitung*>(get_werkauftrag());
	XASSERT(rohr);
	if (!rohr) 
...
bool DialogExcel::setTableRohrdaten(int index, char fname[], bool createTable)
{
	// Table erstellen
...
int DialogExcel::createStrangrohrdata()
{
	Strangrohr* strang = dynamic_cast<Strangrohr*>(get_werkauftrag());
	XASSERT(strang);
	if (!strang) 
...
*/
void DialogExcel::createStrangrohrtabelle()
{
#ifndef _DEBUG
...
void DialogExcel::set_walist(CObList& value)
{
	WaxPart *pt;
...
void DialogExcel::set_walist(MxCObList* walistLocal)
{	
...
void DialogExcel::set_wasublist(CObList& value)
{
	WaxPart *pt;
	for (POSITION pos = value.GetHeadPosition(); pos; )
	{
...
const CObList& DialogExcel::get_sammelauftraege() const
{
	return sammelauftraege;
}

double DialogExcel::get_Stammdaten(int id, int dn, int index)
{
...
CString DialogExcel::insertKopfsatz(Werkauftrag *wa)
{
	// Leitkarte
	CString neueZeile;
	neueZeile.Format("\r\n");
...
bool DialogExcel::writeFile(CString fname, CString text)
{
	CFile file;
...
void DialogExcel::openFile(CString fname, CString text)
{
...
void DialogExcel::sortListe(CObList& liste, bool aufsteigend)
{
	POSITION pos, nextpos; 
...
const CString DialogExcel::get_first_line_of(const CString&  str) const
{
	/**
...
bool DialogExcel::checkLaengeStdElemente(Rohrleitung* vert)
{
	int i=0; 
...
bool DialogExcel::ckeckElevector(Element* ele)
{
